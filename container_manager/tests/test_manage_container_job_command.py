...
class ManageContainerJobCommandTest(TestCase):
    ...
    def test_handle_routes_to_correct_handlers(self):
        ...
        with patch.object(self.command, 'handle_create') as mock_create, \
    ...
    def test_handle_create_creates_job_successfully(self):
        ...
        with patch.object(self.command, 'get_default_user', return_value=self.user), \
    ...
    def test_handle_create_with_inactive_host_raises_error(self):
        ...
        self.assertIn('Docker host "inactive-host" not found or inactive', str(cm.exception))
    ...
    def test_handle_create_with_invalid_env_var_format_raises_error(self):
        ...
        self.assertIn('Invalid environment variable format: INVALID_NO_EQUALS', str(cm.exception))
    ...
    def test_handle_create_with_env_var_containing_equals_in_value(self):
        ...
        with patch.object(self.command, 'get_default_user', return_value=self.user), \
            ...
            self.assertEqual(job.override_environment['CONNECTION_STRING'], 'host=localhost;port=5432')
    ...
    def test_handle_run_executes_pending_job_successfully(self):
        ...
        with patch.object(self.command.executor_factory, 'get_executor', return_value=mock_executor), \
    ...
    def test_handle_cancel_cancels_running_job(self):
        ...
        with patch('container_manager.management.commands.manage_container_job.docker_service') as mock_service:
            options = {'job_id': str(job.id)}
    ...
    def test_handle_cleanup_with_error(self):
        ...
        with patch('container_manager.management.commands.manage_container_job.docker_service') as mock_service:
            mock_service.cleanup_old_containers.side_effect = Exception("Cleanup failed")
    ...
    def test_handle_run_with_exception_raises_command_error(self):
        ...
        with patch.object(self.command.executor_factory, 'get_executor', side_effect=Exception("Test error")):
            with self.assertRaises(CommandError) as cm:
                options = {'job_id': str(job.id)}
                self.command.handle_run(options)
    ...
    def test_show_job_details_with_all_display_methods(self):
        ...
        with patch.object(self.command, '_display_job_header') as mock_header, \
